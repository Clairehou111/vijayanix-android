...
/**
 * this class is used to help DeviceStateMachine process
 * 

 * 
 */
public class DeviceStateMachineHandler implements IDeviceStateMachineHandler
{
    /*
     * Singleton lazy initialization start
     */
    private DeviceStateMachineHandler()
    {
    ...
    {
        static DeviceStateMachineHandler instance = new DeviceStateMachineHandler();
    }
    
    public static DeviceStateMachineHandler getInstance()
    {
    ...
    private final static Logger log = Logger.getLogger(DeviceStateMachineHandler.class);
    ...
    abstract class TaskBase implements ITaskBase
    {
        ...
        {
            DeviceStateMachineExecutor executor = DeviceStateMachineExecutor.getInstance();
            synchronized (this.mFutureLock)
            {
    ...
    void __transformToFail(IDevice device)
    {
        IDeviceStateMachine stateMachine = DeviceStateMachine.getInstance();
        stateMachine.transformState(device, IDeviceStateMachine.Direction.FAIL);
    }
    
    class TaskActivateLocal extends TaskBase implements ITaskActivateLocal
    {
        
        private final IDeviceConfigure mDeviceConfigure;
        
        public TaskActivateLocal(IDeviceConfigure deviceConfigure)
        {
...
//                            mDeviceConfigure.setName(ssid);
        ...
        public void initSucCallableOrRunnable()
        {
            this.mSucRunnable = new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    DeviceStateMachine stateMachine = DeviceStateMachine.getInstance();
                    stateMachine.transformState(mDeviceConfigure, IDeviceStateMachine.Direction.ACTIVATE);
                }
    ...
    class TaskActivateInternet extends TaskBase implements ITaskActivateInternet
    {
        ...
        private volatile IDevice mDeviceResult;
        
        TaskActivateInternet(IDeviceConfigure deviceConfigure)
        {
        ...
        public void initCallableOrRunnable()
        {
            this.mCallable = new Callable<IDevice>()
            {
        ...
        public void initSucCallableOrRunnable()
        {
            this.mSucRunnable = new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    DeviceStateMachine stateMachine = DeviceStateMachine.getInstance();
                    stateMachine.transformState(mDeviceResult, IDeviceStateMachine.Direction.SUC);
                    ...
                    Intent intent = new Intent(VijStrings.Action.ESPTOUCH_DEVICE_REGISTERED);
                    String deviceBssid = mDeviceResult.getBssid();
                    intent.putExtra(VijStrings.Key.DEVICE_BSSID, deviceBssid);
                    String deviceName = mDeviceResult.getName();
                    intent.putExtra(VijStrings.Key.DEVICE_NAME, deviceName);
    ...
    @Override
    public ITaskActivateLocal createTaskActivateLocal(IDeviceConfigure deviceConfigure)
    {
        log.error("createTaskActivateLocal(): " + deviceConfigure);
        return new TaskActivateLocal(deviceConfigure);
    }

    @Override
    public ITaskActivateInternet createTaskActivateInternet(IDeviceConfigure deviceConfigure)
    {
        log.error("createTaskActivateInternet(): " + deviceConfigure);
        return new TaskActivateInternet(deviceConfigure);
    }
