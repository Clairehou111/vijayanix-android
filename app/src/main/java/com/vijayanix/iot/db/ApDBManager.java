...
import com.vijayanix.iot.db.greendao.gen.ApDBDao.Properties;
...
public class ApDBManager implements IApDBManager, ISingletonObject
{
    private final static Logger log = Logger.getLogger(DeviceDBManager.class);
    
    private ApDBDao apDao;
    
    // Singleton Pattern
    private static ApDBManager instance = null;
    
    private ApDBManager(DaoSession daoSession)
    {
    ...
    {   if (instance == null){
	    instance = new ApDBManager(daoSession);
	    log.debug(Thread.currentThread().toString() + " ApDBManager has been initiaited");
	    Log.i("ApDBManager"," ApDBManager has been initiaited");
         }

    }
    
    public static ApDBManager getInstance()
    {
        return instance;
    }
    ...
    {
        Query<ApDB> query = apDao.queryBuilder().where(ApDBDao.Properties.IsLastSelected.eq(true)).build();
        return query.unique();
    }
