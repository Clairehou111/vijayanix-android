...
public class MeshSocketImpl implements MeshSocket
{
    private static final boolean DEBUG = true;
    private static final boolean USE_LOG4J = true;
    private static final Class<?> CLASS = MeshSocketImpl.class;
     ...
     */
    private final BlockingQueue<ProxyTask> mRefreshProxyTaskQueue;
    
    /**
     * the proxy task list which has been sent request
     */
    private final List<ProxyTask> mSentProxyTaskList;
	...
	private MeshResponse mMeshResponse = null;
    
    private final Thread mMainThread;
    
    MeshSocketImpl(InetAddress inetAddress)
    {
        mTargetInetAddr = inetAddress;
        mRefreshProxyTaskQueue = new LinkedBlockingDeque<ProxyTask>();
        mSentProxyTaskList = new ArrayList<ProxyTask>();
    ...
    private boolean isLongSocketSerialExistMap(String targetBssid, int serial)
    {
        synchronized (mLongSocketSerialMap)
        {
	        ...
	        return serialInteger != null && serialInteger.intValue() == serial;
        }
    }
    ...
    private VijSocket open(InetAddress remoteInetAddr)
    {
        VijSocket socket = VijSocket.createEspSocket();
        try
        {
        ...
        for (int retry = 0; !isConnected && retry < SO_CONNECT_RETRY; ++retry)
        {
            ...
            {
                String message = SocketUtil.getStrackTrace(e);
    ...
    private void sendRequestBytes(byte[] requestBytes, String targetBssid, List<String> targetBssidList, int proto)
    {
        ...
        try
        {
            ...
            MeshRequest meshRequest =
                targetBssidList == null ? MeshRequest.createInstance(proto, targetBssid, requestBytes)
                    : MeshRequest.createInstance(proto, targetBssidList, requestBytes);
            requestBytes = meshRequest.getRequestBytes();
            
            // write requestBytes
            SocketUtil.writeBytes(sckOutputStream, requestBytes);
            // flush
            SocketUtil.flush(sckOutputStream);
            refresh();
    ...
    private void loop()
    {
        ...
        if (proxyTask != null && proxyTask != ProxyTaskImpl.CLOSE_PROXYTASK)
        {
    ...
    @Override
    public List<ProxyTask> getRefreshProxyTaskList()
    {
        List<ProxyTask> proxyTaskList = new ArrayList<ProxyTask>();
        for (ProxyTask proxyTask : mRefreshProxyTaskQueue)
        {
            if (proxyTask != ProxyTaskImpl.CLOSE_PROXYTASK && !proxyTask.isFinished())
            {
                proxyTask.updateTimestamp();
    ...
    private void addNewProxyTask(ProxyTask proxyTask)
    {
    ...
    @Override
    public void offer(ProxyTask proxyTask)
    {
        proxyTask.updateTimestamp();
        // check whether the target InetAddress is valid
        if (!proxyTask.getTargetInetAddress().equals(mTargetInetAddr))
        {
            throw new IllegalArgumentException("ProxyTask's target InetAddress is wrong");
        }
        // update the MeshSocketImpl timeout if necessary
    ...
    public synchronized void close()
    {
        if (!mIsClosed)
        {
            ...
            synchronized (mSentProxyTaskList)
            {
                for (ProxyTask proxyTask : mSentProxyTaskList)
                {
                    proxyTask.replyClose();
                    MeshLog.d(DEBUG,
                        USE_LOG4J,
...
//                if (proxyTask != ProxyTaskImpl.CLOSE_PROXYTASK && proxyTask.isExpired())
//                {
...
//                {
//                    int serial =
//                        proxyTask != ProxyTaskImpl.CLOSE_PROXYTASK ? proxyTask.getLongSocketSerial()
...
//            }
            ...
            mRefreshProxyTaskQueue.add(ProxyTaskImpl.CLOSE_PROXYTASK);
            mIsClosed = true;
            mConditionLock.lock();
            mConditionHalfClosed.signalAll();
            mConditionLock.unlock();
            mMainThread.interrupt();
            MeshLog.d(DEBUG, USE_LOG4J, CLASS, "MeshSocketImpl is closed");
        }
    }
    ...
    public boolean isExpired()
    {
	    ...
	    return consume > mTimeout;
    }
    
    /**
     * make the VijSocket keep refresh
     */
    ...
    private boolean replyProxyTask(String targetBssid, byte[] responseBytes)
    {
        ...
        ProxyTask finishedProxyTask = null;
        synchronized (mSentProxyTaskList)
        {
            for (int i = 0; i < mSentProxyTaskList.size(); ++i)
            {
                ProxyTask proxyTask = mSentProxyTaskList.get(i);
...
//                + finishedProxyTask + ", mSentProxyTaskList: " + mSentProxyTaskList);
    ...
    private void receiveResponseBytes()
    {
        ...
        try
        {
            ...
            int byteCount = 4;
            // read first 4 bytes
            SocketUtil.readBytes(sckInputStream, buffer, bufferOffset, byteCount);
            bufferOffset += byteCount;
            // update mMeshResponse
            mMeshResponse = MeshResponse.createInstance(buffer);
            ...
            {
                MeshOption meshOption = mMeshResponse.getMeshOption();
        ...
        {
            String message = SocketUtil.getStrackTrace(e);
            MeshLog.e(DEBUG, USE_LOG4J, CLASS, "receiveResponseBytes() IOException e:" + message + " , so close MeshSocket");
            close();
        }
    }
    ...
    public void checkProxyTaskStateAndProc()
    {
        ...
        List<ProxyTask> expiredSentProxyTaskList = new ArrayList<ProxyTask>();
        synchronized (mSentProxyTaskList)
        {
            for (int i = 0; i < mSentProxyTaskList.size(); ++i)
            {
        ...
        for (ProxyTask proxyTask : expiredSentProxyTaskList)
        {
    ...
    /**
     * get whether the MeshSocket is half closed
     * 
     * @return whether the MeshSocket is half closed
     */
