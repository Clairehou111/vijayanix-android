...
import com.vijayanix.iot.common.net.mdns.MdnsDiscoverUtil;
import com.vijayanix.iot.common.net.udp.UdpBroadcastUtil;
...
public class MeshDiscoverUtil
{
    private static final Logger log = Logger.getLogger(MeshDiscoverUtil.class);
    ...
    private static Set<IOTAddress> __discoverIOTMeshDevicesOnRoot2(IOTAddress rootIOTAddress, String deviceBssid)
    {
        ...
        if (deviceBssid != null)
        {
            IOTAddress iotAddress = MeshNetUtil2.GetTopoIOTAddress5(rootInetAddress, deviceBssid);
            if (iotAddress != null)
            {
        ...
        {
            List<IOTAddress> iotAddressList = MeshNetUtil2.GetTopoIOTAddressList5(rootInetAddress, rootBssid);
    ...
    {
        final IUser user = BUser.getBuilder().getInstance();
        ...
        {
            Future<?> _future = BaseApiUtil.submit(new Runnable()
            {
                @Override
                public void run()
                {
        ...
        for (final IOTAddress rootIOTAddress : rootDeviceSet)
        {
            ...
            Future<Set<IOTAddress>> future = BaseApiUtil.submit(new Callable<Set<IOTAddress>>()
            {
