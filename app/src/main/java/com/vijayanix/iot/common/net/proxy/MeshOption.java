...
/**
 * MeshOption is used for parse MESH OPTION in MESH HEADER
 * 
...
public class MeshOption
{
    ...
    static MeshOption createInstance(byte[] responseBytes, int packageLength, int optionLength)
    {
        try
        {
            return new MeshOption(responseBytes, packageLength, optionLength);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    private MeshOption(byte[] responseBytes, int packageLength, int optionLength)
    {
        if (optionLength <= 0)
        {
    ...
    private MESH_OPTION_TYPE parseType(byte[] responseBytes, int typeOffset, int typeCount)
    {
        ...
        return MESH_OPTION_TYPE.values()[typeValue];
    }
    ...
    {
        int bodyLength = packageLength - MeshPackageUtil.M_HEADER_LEN - optionLength;
        int offset = MeshPackageUtil.M_HEADER_LEN + bodyLength + MeshPackageUtil.M_OPTION_LEN;
        // parse TLV
